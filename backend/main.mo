...
actor {
  ...
  // ALL existing functions should be here
  // Navigation should be handled in the frontend
  ...
  // Added new query to return all projects mapping for a user
  public query ({ caller }) func getAllUserProjects() : async [(ProjectId, Project)] {
    if (not (AccessControl.hasPermission(accessControlState, caller, #user))) {
      Run.trap("Unauthorized: No read access granted to projects");
    };
    let userProjects = getUserProjects(caller);
