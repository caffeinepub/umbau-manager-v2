...
export default function ApplyInvite({ onSuccess }: ApplyInviteProps) {
  ...
  useEffect(() => {
    const processInvite = async () => {
      ...
      if (!identity) {
        setStatus('error');
        setErrorMessage('Sie müssen angemeldet sein, um eine Einladung anzunehmen.');
        return;
      }
      ...
      try {
        ...
        setStatus('error');
        setErrorMessage(error.message || 'Fehler beim Beitreten zum Projekt. Der Code könnte ungültig oder bereits verwendet sein.');
      }
    };
      ...
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription className="text-center">
            {status === 'processing' && 'Bitte warten Sie, während wir Ihre Einladung verarbeiten...'}
        ...
        <CardContent className="flex flex-col items-center gap-4">
          ...
          {status === 'success' && (
            <>
              ...
              <p className="text-sm text-muted-foreground text-center">
                Sie wurden erfolgreich zum Projekt hinzugefügt. Sie werden weitergeleitet...
              </p>
            </>
          )}
          {status === 'error' && (
