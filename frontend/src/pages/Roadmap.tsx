...
import { useGetAllProjects, useUpdateProject, useDeleteProject, useGetAllContacts, useCreateProject, useGetCallerUserProfile, useGetAllCostItems, useGetAllTasks } from '../hooks/useQueries';
...
export default function Roadmap() {
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    // Save the newly created project ID to session
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {kategorienOptions.map((cat, idx) => (
              <Button
                ...
                style={{
                  backgroundColor: selectedCategory === cat ? getCategoryColor(idx) : undefined,
                  borderColor: getCategoryColor(idx),
                  color: selectedCategory === cat ? 'white' : getCategoryColor(idx),
                }}
              >
                ...
                return (
                  <Card key={project.id} className="overflow-hidden">
                    <Collapsible open={isExpanded} onOpenChange={() => toggleProjectExpansion(project.id)}>
                      <CardHeader className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => toggleProjectExpansion(project.id)}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 flex-1">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                                ...
                                <Badge variant="outline">{project.kategorie}</Badge>
                                ...
                                {project.verantwortlicherKontakt && (
                                  <span>Kontakt: {getContactName(project.verantwortlicherKontakt)}</span>
                                )}
                              </div>
                            </div>
                          </div>
                      ...
                      <CollapsibleContent>
                        <CardContent className="pt-0">
                          <div className="text-sm text-muted-foreground">
                            <p>Weitere Details zum Projekt können hier angezeigt werden.</p>
                          </div>
                        </CardContent>
                      </CollapsibleContent>
                    </Collapsible>
      ...
      <BaseDialog
        ...
        description="Bearbeiten Sie die Projektdetails und Kostenpunkte"
      >
        <form onSubmit={handleUpdateProject} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            <Button type="submit" disabled={updateProject.isPending}>
              {updateProject.isPending ? 'Speichere...' : 'Änderungen speichern'}
            </Button>
          </div>
        </form>
      </BaseDialog>
