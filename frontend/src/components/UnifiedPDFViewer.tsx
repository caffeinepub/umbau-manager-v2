...
import { ChevronLeft, ChevronRight, ZoomIn, ZoomOut, X, Download, Maximize, Minimize } from 'lucide-react';
...
const PDFCanvas = memo(({ pdfDoc, pageNumber, scale }: { pdfDoc: any; pageNumber: number; scale: number }) => {
...
export const UnifiedPDFViewer = memo(({ open, onOpenChange, pdfUrl, title = 'PDF Dokument', disableDownload = false }: UnifiedPDFViewerProps) => {
  ...
  useEffect(() => {
    if (open && pdfUrl && window.pdfjsLib) {
      ...
      if (cachedData) {
        window.pdfjsLib.getDocument({ data: cachedData }).promise
          .then((pdf: any) => {
            setPdfDoc(pdf);
  ...
  }, [open, pageNumber, numPages, scale, isFullscreen]);
            ...
            <div className="flex items-center gap-2">
              {isFullscreen && (
                <>
                  ...
                  <span className="text-sm whitespace-nowrap min-w-[60px] text-center">{Math.round(scale * 100)}%</span>
                  <Button size="sm" variant="outline" onClick={zoomIn} disabled={scale >= 3.0}>
              ...
              <Button size="sm" variant="outline" onClick={toggleFullscreen} title="Vollbild (F)">
            ...
            <div className="h-full flex items-center justify-center">
              <div className="text-center space-y-4">
                {showThumbnail && pdfUrl ? (
                  <PDFThumbnail pdfUrl={pdfUrl} className="mx-auto max-w-[200px]" />
                ) : (
